...
namespace e {
	...
	void flandmark_write_model(const char* filename, FLANDMARK_Model* model)
	{
		...
		{
			LOGE(_T("open flandmark model data failed: %s\n"), filename);
			exit(1);
		}
		...
		for (int idx = 0; idx < 3; ++idx)
		{
			...
			LOGD(_T("model->data.options.PSIG_ROWS[%d] = %d; model->data.options.PSIG_COLS[%d] = %d; \n"),
				idx, model->data.options.PSIG_ROWS[idx], idx, model->data.options.PSIG_COLS[idx]);
		}

		// write model->W
		LOGI(_T("Writing model->W to file... "));
		if (fwrite(model->W, model->W_ROWS * sizeof(double), 1, fout) != 1)
		{
			fclose(fout);
			LOGE(_T("write flandmark model : %s\n"), filename);
			exit(1);
		}

		// write model->mapTable
		LOGI(_T("Writing model->data.mapTable to file... "));
		if (fwrite(model->data.mapTable, model->data.options.M * 4 * sizeof(int), 1, fout) != 1)
		{
			fclose(fout);
			LOGD(_T("Error writing file %s\n"), filename);
			exit(1);
		}

		// write model->data.lbp ---------------------------------------------------
		LOGI(_T("Writing model->data.lbp to file... \n"));
		for (int idx = 0; idx < model->data.options.M; ++idx)
		{
			//printf("lbp[%d]... ", idx);
			// lbp{idx}.winSize
		...
		LOGI(_T("Writing model->data.options.S to file... "));
	...
	EError_T flandmark_check_model(FLANDMARK_Model* model, FLANDMARK_Model* tst)
	{
		...
		LOGI(_T("Checking mode->data.options.M..."));
		flag = true;
		...
		LOGI(_T("Checking mode->data.options.bw..."));
		flag = true;
		...
		LOGI(_T("Checking mode->data.options.bw_margin..."));
		flag = true;
		...
		LOGI(_T("Checking model->data.mapTable... "));
		flag = true;
		for (int i = 0; i < M * 4; ++i)
		{
		...
		LOGI(_T("Checking model->data.options.S... "));
		flag = true;
		for (int i = 0; i < 4 * M; ++i)
		{
		...
		for (int psig_idx = 0; psig_idx < 3; ++psig_idx)
		{
			...
			flag = true;
			LOGI(_T("Checking model->data.options.PsiGS%d\n"), psig_idx);
			LOGI(_T("options.PSIG_ROWS[%d]; options.PSIG_COLS[%d]... "), psig_idx, psig_idx);
			...
			LOGI(_T("options.PsiGS%d..."), psig_idx);
			for (int idx = 0; idx < tsize; ++idx)
			{
	...
	void flandmark_get_psi_mat(FLANDMARK_PSI* Psi, FLANDMARK_Model* model, int lbpidx)
	{
		...
		if (Features == NULL)
		{
			LOGE(_T("Not enough memory for LBP features.\n"));
			exit(1);
		}
		...
		if (win == NULL)
		{
			LOGE(_T("Not enough memory for cropped_window.\n"));
			exit(1);
		}
